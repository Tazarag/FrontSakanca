...
import { Camera, Compass, Code, Car, Cpu, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function DetailedServicesSection() {
    ...
    // Listen for custom slide selection events from ServicesSection
    useEffect(() => {
    ...
    const handleSlideChange = (newIndex: number) => {
        setAnimate(false);
        setBgIndex(newIndex); // Change background instantly for instant responsiveness
        setTimeout(() => {
            setActiveIndex(newIndex);
            setAnimate(true);
        }, 350); // Wait for fade-out transition to complete
    };

    const nextSlide = () => {
        const nextIndex = (activeIndex + 1) % servicesDetailList.length;
        handleSlideChange(nextIndex);
    };

    const prevSlide = () => {
        const prevIndex = (activeIndex - 1 + servicesDetailList.length) % servicesDetailList.length;
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-black/90 via-[#0a1334]/80 to-black/95 z-0 pointer-events-none" />

            <div className="max-w-6xl mx-auto w-full relative z-10 flex flex-col justify-center min-h-[500px]">

                {/* Main Slide Layout: Title & Logo Side-by-Side */}
                <div className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-24 mb-12">

                    {/* Left Side: Right-Aligned Title emerging from inside the logo */}
                    <div className={`flex flex-col justify-center text-right min-w-[280px] sm:min-w-[320px] md:min-w-[480px] transition-all ${animate ? 'opacity-100 translate-x-0 scale-100 blur-none duration-[1000ms] [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] [transition-delay:1000ms]' : 'opacity-0 translate-x-[200px] sm:translate-x-[280px] md:translate-x-[360px] scale-75 blur-[8px] duration-[250ms] ease-in'}`}>
                        <h2 className="text-[#E6DAC3] font-bold text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-[1.2] tracking-[0.2em] uppercase select-none drop-shadow-[0_4px_10px_rgba(0,0,0,0.4)] text-right">
                            ...
                            <span className="block mt-2 text-[#E6DAC3]">{activeService.titleLine2}</span>
                        </h2>
                    </div>

                    {/* Right Side: Large Rounded Glowing Logo Card with Scale & Rotation Animation */}
                    <div className={`flex justify-center items-center shrink-0 transition-all ${animate ? 'opacity-100 translate-x-0 scale-100 rotate-0 blur-none duration-[1200ms] [transition-timing-function:cubic-bezier(0.16,1,0.3,1)]' : 'opacity-0 translate-x-12 scale-90 rotate-3 blur-[4px] duration-[250ms] ease-in'}`}>
                ...
                {/* Bottom Row: Detailed Description emerging from below with fade-in */}
                <div className={`max-w-4xl mx-auto w-full mb-8 transition-all ${animate ? 'opacity-100 translate-y-0 blur-none duration-[900ms] [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] [transition-delay:1600ms]' : 'opacity-0 translate-y-16 blur-[4px] duration-[250ms] ease-in'}`}>
                ...
                {/* Bullet Indicators / Pagination dots */}
                    ...
                    {servicesDetailList.map((_, index) => (
                        <button
                            ...
                            className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${index === activeIndex
                                ? 'w-10 bg-[#E6DAC3] shadow-[0_0_8px_rgba(230,218,195,0.6)]'
                ...
                <div className="mt-12 flex justify-center gap-4 z-30 relative">
                    <a
                        href="#services"
                    ...
                    >
                        <span className="lang-text">{t('btn_view_project')}</span>
                    </a>
                </div>
